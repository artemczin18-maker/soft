...
def pay_usdt(call):
    ...
    if inv:
        ...
        bot.edit_message_text(
            f"Счёт на {price}RUB (~{round(price/90,2)} USDT)\nТариф: {TARIFFS[tariff]['name']}\nДействует 30 минут",
...
def check_usdt(call):
    ...
    if status == "paid":
        ...
        if uid:
            ...
            USERS[uid]["keys"].append({"key": key, "tariff": TARIFFS[tariff]["name"], "end": USERS[uid]["sub_end"]})
